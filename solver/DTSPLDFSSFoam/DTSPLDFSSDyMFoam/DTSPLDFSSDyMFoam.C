...
int main(int argc, char *argv[])
{
    ...
    while (runTime.run())
    {
        ...
        // Do any mesh changes
/*        const dictionary& motionControl =
        mesh.solutionDict().subDict("motionControl");

    	scalar motionDeltaT(readScalar(motionControl.lookup("deltaT")));
	if(motionDeltaT0 >= motionDeltaT)
	{
            mesh.update();
	    motionDeltaT0 = 0.0;
	}
	...
	{
	    motionDeltaT0 += runTime.deltaT().value();
	}
*/
	mesh.update();
        if (mesh.moving())
